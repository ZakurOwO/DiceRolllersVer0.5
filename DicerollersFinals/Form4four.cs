...
namespace DicerollersFinals
{
    public partial class Form4 : Form
    {
        ...
        private void InitializeRevealTimer()     // Initializes the timer for revealing dice one by one
        {
            ...
            revealTimer.Interval = 2000; // 1 second between reveals
        ...
        public void MultiplierReappear()
        {
            MultiplierLabel1.Visible = true; //If error occured in rolling, the invisible multiplier will reappear
        ...
        private void rollButton_Click(object sender, EventArgs e) // Handles logic when the "Roll" button is clicked
        {
            if (!isRolling)
            {
                ...
                if (!int.TryParse(betAmount.Text, out bet) || bet <= 499) //Bet must be at least 500
                {
                    MultiplierReappear();
                ...
                if (bet > currentBalance) //Bet must not exceed current balance
                {
                    MultiplierReappear();
        ...
        private async void DisplayRollOutcome()
        {
        ...
        {
            MultiplierLabel1.Text = "0.00";
            MultiplierLabel2.Text = "0.00";
            MultiplierLabel3.Text = "0.00";
            MultiplierLabel1.Visible = true;
            MultiplierLabel2.Visible = true;
            MultiplierLabel3.Visible = true;
        }
        ...
        private void ShowDice(int d1, int d2, int d3) // Displays all three dice with their values
        {
            PictureBox[] dicePics = { PicDice1, PicDice2, PicDice3 };
            int[] values = { d1, d2, d3 };
            MultiplierSoundFX(); //Multiplier soundfx
            for (int i = 0; i < 3; i++)
            {
